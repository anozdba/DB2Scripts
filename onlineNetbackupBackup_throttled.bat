...
REM
REM $Id: onlineNetbackupBackup_throttled.bat,v 1.1 2010/08/26 22:41:24 db2admin Exp db2admin $
REM
REM Description:
...
REM
REM ChangeLog:
REM $Log: onlineNetbackupBackup_throttled.bat,v $
REM Revision 1.1  2010/08/26 22:41:24  db2admin
...
:rc2_00
    ...
    db2 backup database %DB% ONLINE LOAD %LOAD%  UTIL_IMPACT_PRIORITY 90 include logs >>%OUTPUT_LOG%
    set RC3=%errorlevel%
    if %RC3% == 0 goto rc3_00
      set SEND_EMAIL=1
      echo " Throttled DB2 Backup failed with RC = %RC3% " >>%OUTPUT_FILE%
      goto cleanup
:rc3_00
      echo " Throttled Backup of %DB% successful " >>%OUTPUT_FILE%
      set SEND_FILE=1
      echo 
...
if NOT %SEND_EMAIL% == 0 (
  ...
  REM Send an email
  if "%DB%" EQU "" (
    cscript.exe c:\udbdba\scripts\sendEmail.vbs "%machine% - Throttled Online Backup of UNKNOWN failed" mpl_it_dba_udb@KAGJCM.com.au NONE "results from the online backup of %machine% / %DB2INSTANCE% / %DB% :" %OUTPUT_FILE%  %OUTPUT_LOG%
  ) ELSE (
    ...
    cscript.exe c:\udbdba\scripts\sendEmail.vbs "%machine% - Throttled Online Backup of %DB% failed" mpl_it_dba_udb@KAGJCM.com.au NONE "results from the online backup of %machine% / %DB2INSTANCE% / %DB% :" %OUTPUT_FILE%  %OUTPUT_LOG%
  )
)

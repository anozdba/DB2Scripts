...
if NOT %SEND_EMAIL% == 0 (
  if "%DB%" EQU "" (
    ...
    cscript.exe c:\udbdba\scripts\sendEmail.vbs "%machine% - Online Backup of %DB% failed" mpl_it_dba_udb@KAGJCM.com.au NONE "results from the online backup of %machine% / %DB2INSTANCE% / %DB% :" %OUTPUT_FILE%  %OUTPUT_LOG%
  )
)
